...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_ConceptoPago_MigracionTP_U_InicializarEstadoValidacion')
	DROP PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_MigracionTP_U_InicializarEstadoValidacion]
GO


CREATE PROCEDURE USP_Obligaciones_ConceptoPago_MigracionTP_U_InicializarEstadoValidacion	
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID	tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_ConceptoPago_MigracionTP_U_InicializarEstadoValidacion @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	BEGIN TRANSACTION
	BEGIN TRY 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_MarcarConceptosPagoEliminados')
	DROP PROCEDURE [dbo].[USP_U_MarcarConceptosPagoEliminados]
GO
...
CREATE PROCEDURE USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_12_13_Repetidos	
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_12_13_Repetidos @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	...
	BEGIN TRY 
		...
		SET		B_Migrable = 0,
				D_FecEvalua = @D_FecProceso
		WHERE	Id_cp IN (SELECT Id_cp FROM TR_Cp_Pri WHERE I_ProcedenciaID = @I_ProcedenciaID AND Eliminado = 0 
						  GROUP BY Id_cp HAVING COUNT(Id_cp) > 1
						  -- UNION
						  --SELECT ID_CP FROM TR_Cp_Pri WHERE I_ProcedenciaID = @I_ProcedenciaID AND ELIMINADO = 1 
						  --GROUP BY ID_CP HAVING COUNT(ID_CP) > 1
						 )
				AND Eliminado = 0 
				AND I_RowID = IIF(@I_RowID IS NULL, I_RowID, @I_RowID)
		...
		IF EXISTS (SELECT Id_cp FROM TR_Cp_Pri WHERE I_ProcedenciaID = @I_ProcedenciaID GROUP BY Id_cp HAVING COUNT(Id_cp) > 1)
		BEGIN
			UPDATE	TR_Cp_Pri
			SET		B_Migrable = 0,
					D_FecEvalua = @D_FecProceso
			WHERE	Id_cp IN (SELECT Id_cp FROM TR_Cp_Pri WHERE I_ProcedenciaID = @I_ProcedenciaID
							  GROUP BY Id_cp HAVING COUNT(Id_cp) > 1)
					AND Eliminado = 1
					AND I_RowID = IIF(@I_RowID IS NULL, I_RowID, @I_RowID)
		END
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_MarcarConceptosPagoConAnioDiferenteCuotaPago')
	DROP PROCEDURE [dbo].[USP_U_MarcarConceptosPagoConAnioDiferenteCuotaPago]
GO

IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_15_AnioDiferenteCuotaPago')
	DROP PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_15_AnioDiferenteCuotaPago]
GO

CREATE PROCEDURE USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_15_AnioDiferenteCuotaPago	
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_15_AnioDiferenteCuotaPago @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	...
	BEGIN TRY 
		...
		SET @I_Observados_AnioDif = (SELECT COUNT(*) FROM TI_ObservacionRegistroTabla 
									 WHERE I_ObservID = @I_ObservID_AnioDif 
										   AND I_TablaID = @I_TablaID 
										   AND I_ProcedenciaID = @I_ProcedenciaID
										   AND I_FilaTablaID = IIF(@I_RowID IS NULL, I_FilaTablaID, @I_RowID)
										   AND B_Resuelto = 0
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_MarcarConceptosPagoConPeriodoDiferenteCuotaPago')
	DROP PROCEDURE [dbo].[USP_U_MarcarConceptosPagoConPeriodoDiferenteCuotaPago]
GO

IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_17_ConPeriodoDiferenteCuotaPago')
	DROP PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_17_ConPeriodoDiferenteCuotaPago]
GO


CREATE PROCEDURE USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_17_ConPeriodoDiferenteCuotaPago	
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_ConceptoPago_MigracionTP_U_Validar_17_ConPeriodoDiferenteCuotaPago @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	...
	BEGIN TRY 
		...
		SET @I_Observados_PerDif = (SELECT COUNT(*) FROM TI_ObservacionRegistroTabla 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_ConceptoPago_MigracionTP_U_AsignarIdEquivalencias')
	DROP PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_MigracionTP_U_AsignarIdEquivalencias]
GO

CREATE PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_MigracionTP_U_AsignarIdEquivalencias]
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_ConceptoPago_CtasPorCobrar_IU_GrabarTablaCatalogoConceptos')
	DROP PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_CtasPorCobrar_IU_GrabarTablaCatalogoConceptos]
GO

CREATE PROCEDURE USP_Obligaciones_ConceptoPago_CtasPorCobrar_IU_GrabarTablaCatalogoConceptos	
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
AS
--declare @B_Resultado  bit,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_ConceptoPago_CtasPorCobrar_IU_GrabarTablaCatalogoConceptos @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_ConceptoPago_MigracionTP_CtasPorCobrar_IU_MigrarData')
	DROP PROCEDURE [dbo].[USP_Obligaciones_ConceptoPago_MigracionTP_CtasPorCobrar_IU_MigrarData]
GO

CREATE PROCEDURE USP_Obligaciones_ConceptoPago_MigracionTP_CtasPorCobrar_IU_MigrarData
...
--			@I_ProcedenciaID tinyint = 3,
--			@T_Message nvarchar(4000)
--exec USP_Obligaciones_ConceptoPago_MigracionTP_CtasPorCobrar_IU_MigrarData @I_ProcesoID, @I_AnioIni, @I_AnioFin, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_ActualizarEquivalenciaCtasPorCobrar')
	DROP PROCEDURE [dbo].[USP_U_ActualizarEquivalenciaCtasPorCobrar]
GO
