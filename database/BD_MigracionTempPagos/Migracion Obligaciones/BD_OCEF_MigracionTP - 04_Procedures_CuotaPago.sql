...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_InicializarEstadoValidacion')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_InicializarEstadoValidacion]
GO


CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_InicializarEstadoValidacion	
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID	tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_InicializarEstadoValidacion @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	BEGIN TRANSACTION
	BEGIN TRY 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_MarcarCuotaPagoEliminada')
	DROP PROCEDURE [dbo].[USP_U_MarcarCuotaPagoEliminada]
GO
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_03_RepetidosActivo')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_03_RepetidosActivo]
GO

CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_03_RepetidosActivo
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_03_RepetidosActivo @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN	
	DECLARE @D_FecProceso datetime = GETDATE() 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_04_RepetidosEliminado')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_04_RepetidosEliminado]
GO

CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_04_RepetidosEliminado
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@I_ProcedenciaID tinyint = 3,
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_04_RepetidosEliminado @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN	
	DECLARE @D_FecProceso datetime = GETDATE() 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_AsignarAnioCuotaPago')
	DROP PROCEDURE [dbo].[USP_U_AsignarAnioCuotaPago]
GO

IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_05_VariosAniosAsignados')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_05_VariosAniosAsignados]
GO

CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_05_VariosAniosAsignados
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@T_SchemaDB		 varchar(20),
	@B_Resultado  bit output,
...
--		@T_SchemaDB		 varchar(20) = 'euded',
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_05_VariosAniosAsignados @I_RowID, @I_ProcedenciaID, @T_SchemaDB, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	DECLARE @T_SQL nvarchar(max)
	...
	BEGIN TRY
		...
		--CUOTAS DE PAGO SIN A�O EN CP_PRI PERO CON A�O EN EC_OBL
					  ...
					  SELECT DISTINCT o.CUOTA_PAGO, o.ANO FROM BD_OCEF_TemporalPagos.' + @T_SchemaDB + '.ec_obl o 
							 INNER JOIN (SELECT cd.CUOTA_PAGO FROM TR_Cp_Des cd
											...
											WHERE anio_cuota IS NULL
												  AND cd.I_ProcedenciaID = ' + CAST(@I_ProcedenciaID as varchar) + ' 
												  AND cd.I_RowID = ' + IIF(@I_RowID IS NULL, 'cd.I_RowID', CAST(@I_RowID as varchar))  + ') cdca
		...
		USING 	(SELECT	@I_ObsMasUnAnio AS I_ObservID, @I_TablaID AS I_TablaID, I_RowID AS I_FilaTablaID, @D_FecProceso AS D_FecRegistro FROM TR_Cp_Des
				 WHERE CUOTA_PAGO IN (SELECT cuota_pago FROM ##cuota_anio GROUP BY cuota_pago HAVING COUNT(*) > 1)
					   ...
					   AND B_MantenerAnio = 0
					   AND Eliminado = 0) AS SRC
		ON TRG.I_ObservID = SRC.I_ObservID AND TRG.I_TablaID = SRC.I_TablaID AND TRG.I_FilaTablaID = SRC.I_FilaTablaID
		...
		WHEN NOT MATCHED BY TARGET AND SRC.I_FilaTablaID = IIF(@I_RowID IS NULL, SRC.I_FilaTablaID, @I_RowID) THEN
			INSERT (I_ObservID, I_TablaID, I_FilaTablaID, D_FecRegistro, I_ProcedenciaID)
			VALUES (SRC.I_ObservID, SRC.I_TablaID, SRC.I_FilaTablaID, SRC.D_FecRegistro, @I_ProcedenciaID)
		WHEN NOT MATCHED BY SOURCE AND TRG.I_ObservID = @I_ObsMasUnAnio AND TRG.I_ProcedenciaID = @I_ProcedenciaID AND TRG.I_FilaTablaID = IIF(@I_RowID IS NULL, TRG.I_FilaTablaID, @I_RowID) THEN
			UPDATE SET D_FecResuelto = GETDATE(),
					   B_Resuelto = 1;
		...
		SET		B_Migrable = 0,
				D_FecEvalua = @D_FecProceso
		WHERE	I_RowID IN (SELECT I_FilaTablaID FROM TI_ObservacionRegistroTabla 
							WHERE I_TablaID = @I_TablaID 
								  AND I_ProcedenciaID = @I_ProcedenciaID 
								  AND B_Resuelto = 0 
								  AND I_ObservID = @I_ObsMasUnAnio)
				AND Eliminado = 0
				AND (I_RowID = IIF(@I_RowID IS NULL, I_RowID, @I_RowID))
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_06_SinAnioAsignado')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_06_SinAnioAsignado]
GO

CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_06_SinAnioAsignado
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@T_SchemaDB		 varchar(20),
	@B_Resultado  bit output,
...
--		@T_SchemaDB		 varchar(20) = 'euded',
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_06_SinAnioAsignado @I_RowID, @I_ProcedenciaID, @T_SchemaDB, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	DECLARE @T_SQL nvarchar(max)
	...
	BEGIN TRY
		...
		WHEN NOT MATCHED BY TARGET AND SRC.I_FilaTablaID = IIF(@I_RowID IS NULL, SRC.I_FilaTablaID, @I_RowID) THEN
			INSERT (I_ObservID, I_TablaID, I_FilaTablaID, D_FecRegistro, I_ProcedenciaID)
			VALUES (SRC.I_ObservID, SRC.I_TablaID, SRC.I_FilaTablaID, SRC.D_FecRegistro, @I_ProcedenciaID)
		WHEN NOT MATCHED BY SOURCE AND TRG.I_ObservID = @I_ObsSinAnio AND TRG.I_ProcedenciaID = @I_ProcedenciaID AND TRG.I_FilaTablaID = IIF(@I_RowID IS NULL, TRG.I_FilaTablaID, @I_RowID) THEN
			UPDATE SET D_FecResuelto = GETDATE(),
					   B_Resuelto = 1;
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_U_AsignarPeriodoCuotaPago')
	DROP PROCEDURE [dbo].[USP_U_AsignarPeriodoCuotaPago]
GO

IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_07_VariosPeriodosAsignados')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_07_VariosPeriodosAsignados]
GO

CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_07_VariosPeriodosAsignados
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@T_SchemaDB		 varchar(20),
	@B_Resultado  bit output,
...
--		@T_SchemaDB		 varchar(20) = 'euded',
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_07_VariosPeriodosAsignados @I_RowID, @I_ProcedenciaID, @T_SchemaDB, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	DECLARE @T_SQL nvarchar(max)
	...
	BEGIN TRY
		...
		WHEN NOT MATCHED BY TARGET AND SRC.I_FilaTablaID = IIF(@I_RowID IS NULL, SRC.I_FilaTablaID, @I_RowID) THEN
			INSERT (I_ObservID, I_TablaID, I_FilaTablaID, D_FecRegistro, I_ProcedenciaID)
			VALUES (SRC.I_ObservID, SRC.I_TablaID, SRC.I_FilaTablaID, SRC.D_FecRegistro, @I_ProcedenciaID)
		WHEN NOT MATCHED BY SOURCE AND TRG.I_ObservID = @I_ObsMasUnPeriodo AND TRG.I_ProcedenciaID = @I_ProcedenciaID AND TRG.I_FilaTablaID = IIF(@I_RowID IS NULL, TRG.I_FilaTablaID, @I_RowID) THEN
			UPDATE SET D_FecResuelto = GETDATE(),
					   B_Resuelto = 1;
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_08_SinPeriodoAsignado')
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_08_SinPeriodoAsignado]
GO

CREATE PROCEDURE USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_08_SinPeriodoAsignado
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@T_SchemaDB		 varchar(20),
	@B_Resultado  bit output,
...
--		@T_SchemaDB		 varchar(20) = 'euded',
--		@T_Message	  nvarchar(4000)
--exec USP_Obligaciones_CuotaPago_MigracionTP_U_Validar_08_SinPeriodoAsignado @I_RowID, @I_ProcedenciaID, @T_SchemaDB, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE() 
	DECLARE @T_SQL nvarchar(max)
	...
	BEGIN TRY
		...
		WHEN NOT MATCHED BY TARGET AND SRC.I_FilaTablaID = IIF(@I_RowID IS NULL, SRC.I_FilaTablaID, @I_RowID) THEN
			INSERT (I_ObservID, I_TablaID, I_FilaTablaID, D_FecRegistro, I_ProcedenciaID)
			VALUES (SRC.I_ObservID, SRC.I_TablaID, SRC.I_FilaTablaID, SRC.D_FecRegistro, @I_ProcedenciaID)
		WHEN NOT MATCHED BY SOURCE AND TRG.I_ObservID = @I_ObsSinPeriodo AND TRG.I_ProcedenciaID = @I_ProcedenciaID AND TRG.I_FilaTablaID = IIF(@I_RowID IS NULL, TRG.I_FilaTablaID, @I_RowID) THEN
			UPDATE SET D_FecResuelto = GETDATE(),
					   B_Resuelto = 1;
		...
		WHERE	I_RowID IN (SELECT I_FilaTablaID FROM TI_ObservacionRegistroTabla 
							WHERE I_TablaID = @I_TablaID 
									AND I_ProcedenciaID = @I_ProcedenciaID 
									AND B_Resuelto = 0 
									AND I_ObservID = @I_ObsSinPeriodo)
				AND Eliminado = 0
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_Obligaciones_CuotaPago_MigracionTP_CtasPorCobrar_IU_MigrarData')
BEGIN
	DROP PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_CtasPorCobrar_IU_MigrarData]
END
GO

CREATE PROCEDURE [dbo].[USP_Obligaciones_CuotaPago_MigracionTP_CtasPorCobrar_IU_MigrarData]
...
BEGIN
	...
	BEGIN TRY 
		...
		SET @T_Message = CAST(@I_Proc_Inserted AS varchar) + ' procesos insertados | ' + CAST(@I_Proc_Updated AS varchar) + ' procesos actualizados ' 
						 ...
						 + ' | ' + CAST(@I_CtaCat_Inserted AS varchar) + ' cuenta-categor�a insertados | ' + CAST(@I_CtaCat_Updated AS varchar) + '  cuenta-categor�a actualizados'
	END TRY
	BEGIN CATCH
		ROLLBACK TRANSACTION;
		SET @B_Resultado = 0
