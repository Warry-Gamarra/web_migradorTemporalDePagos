...
WHERE alu.I_ProcedenciaID = 1 AND obs.I_ObservID = 30
	...
	SELECT	A.I_PersonaID, LTRIM(RTRIM(REPLACE(P.C_NumDNI,' ', ' '))), P.C_CodTipDoc, P.T_ApePaterno, P.T_ApeMaterno, P.T_Nombre, 
	...
	INSERT INTO ##TEMP_Persona (I_PersonaID, C_NumDNI, C_CodTipDoc, T_ApePaterno, T_ApeMaterno, T_Nombre, C_Sexo, D_FecNac)
	SELECT	DISTINCT @I_TempPersonaID + ROW_NUMBER() OVER(ORDER BY TA.T_ApePaterno) AS I_PersonaID, LTRIM(RTRIM(REPLACE(TA.C_NumDNI,' ', ' '))), 
