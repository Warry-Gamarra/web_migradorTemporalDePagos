...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_MigracionTP_Tasas_IU_CuotaPago_CopiarTablaTemporalPagos')
BEGIN
	DROP PROCEDURE dbo.USP_MigracionTP_Tasas_IU_CuotaPago_CopiarTablaTemporalPagos
END
GO

CREATE PROCEDURE dbo.USP_MigracionTP_Tasas_IU_CuotaPago_CopiarTablaTemporalPagos
(
...
--exec USP_MigracionTP_Tasas_IU_CuotaPago_CopiarTablaTemporalPagos @I_ProcedenciaID, @T_Codigo_bnc, @B_Resultado output, @T_Message output
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_MigracionTP_Tasas_U_CuotaPago_InicializarEstadosValidacion')
	DROP PROCEDURE [dbo].[USP_MigracionTP_Tasas_U_CuotaPago_InicializarEstadosValidacion]
GO

CREATE PROCEDURE USP_MigracionTP_Tasas_U_CuotaPago_InicializarEstadosValidacion	
	@I_RowID	  int = NULL,
	@I_ProcedenciaID tinyint,
	@B_Resultado  bit output,
	@T_Message	  nvarchar(4000) OUTPUT	
...
--		@T_Message	  nvarchar(4000)
--exec USP_MigracionTP_Tasas_U_CuotaPago_InicializarEstadosValidacion @I_RowID, @I_ProcedenciaID, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	BEGIN TRANSACTION
	BEGIN TRY 
...
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'USP_MigracionTP_Tasas_IU_CuotaPago_AsignarCategoríaPagoCtasPorCobrar')
BEGIN
	DROP PROCEDURE dbo.USP_MigracionTP_Tasas_IU_CuotaPago_AsignarCategoríaPagoCtasPorCobrar
END
GO

CREATE PROCEDURE dbo.USP_MigracionTP_Tasas_IU_CuotaPago_AsignarCategoríaPagoCtasPorCobrar
(
	@I_ProcedenciaID tinyint,
	@T_Codigo_bnc	 varchar(250),
	@B_Resultado	 bit output,
...
--		@T_Codigo_bnc		nvarchar(250) = N'',
--		@T_Message			nvarchar(4000)
--exec USP_MigracionTP_Tasas_IU_CuotaPago_AsignarCategoríaPagoCtasPorCobrar @I_ProcedenciaID, @T_Codigo_bnc, @B_Resultado output, @T_Message output
--select @B_Resultado as resultado, @T_Message as mensaje
BEGIN
	DECLARE @D_FecProceso datetime = GETDATE()
